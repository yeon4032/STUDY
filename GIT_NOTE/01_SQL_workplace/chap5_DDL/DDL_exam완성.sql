...
delete table ANIMAL_INS;
시퀀스와 레코드를 삭제 후 
sequence 생성, 레코드 삽입을 다시하세요. 
...
ORDER BY AID;
-- SUBQUERY : 입양 간 동물의 ID를 제외한 나머지 ID를 입양동물 테이블에서 선택 
...
FROM ANIMAL_INS
WHERE aid NOT IN (SELECT aid FROM ANIMAL_OUTS) AND  ROWNUM <=3;
-- 입양 못간 동물 조회  & 가장 오래 보호소 있었던 동물 3마리
