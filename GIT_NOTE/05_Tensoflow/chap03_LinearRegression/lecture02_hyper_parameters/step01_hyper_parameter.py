...
iter_size = 1000 # 학습횟수 
'''
1차 테스트: Ir = 0.001, iter size = 100 -> 안정적인 형태 
2차 테스트: Ir = 0.05, iter size = 100 -> 최소점 수렴 속도 빠름  ( leaning rate 증가시)
3차 테스트: Ir = 0.0001, iter size = 100 -> 최소점 수렴 속도 느림 (학습 횟수를 늘릴 필요가 있다.)  <- learning rate 이 낮을시 
4차 테스트: Ir = 0.0001, iter size = 1000 -> 최소점 수렴 속도 느림 이 대안으로 학습 획수 증가
'''
...
# 5. 비용 함수 정의: 
def loss_fn_l1() : # MAE 이용 #  L1 loss function : Lasso 회귀  
    y_pred = linear_model(X) # 예측치 : 회귀방정식  
    err = Y - y_pred # 오차 
    loss = tf.reduce_mean(tf.abs(err)) 
    return loss

def loss_fn_l2() : # MSE 이용 #  L2 loss function : Lidge 회귀  
    y_pred = linear_model(X) # 예측치 : 회귀방정식  
    err = Y - y_pred # 오차 
