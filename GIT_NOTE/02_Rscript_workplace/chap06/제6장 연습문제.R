...
# (2) 서브셋 생성 
mtcars_sub<-subset(mtcars,qsec>=14.500 & qsec<=20.22)
mtcars_sub
help(subset)
...
# 04. dataset3의 gender 칼럼을 대상으로 1->"남자", 2->"여자" 형태로 코딩 변경하여 
...
dataset2$age3[dataset2$age>=30]<-3

dataset2[c("age","age2","age3")]

# 06. 정제된 dataset3를 대상으로 작업 디렉터리(c:/itwill/2_Rwork/output)에 cleanData.csv 파일명으로 
# 따옴표와 행 이름을 제거하여 저장하고, new_data변수로 읽어오시오.



# (1) 정제된 데이터 저장
wirte.csv(dataset2,'c')
# (2) 저장된 파일 불러오기/확인
...
names(return_data) <- c('user_id','return_code1','return_code2','return_code3','return_code4')
head(return_df)
